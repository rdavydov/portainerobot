...
def get_stack_info(update, context):
    ...
    if str(update.message.chat_id) != CHAT_ID:
        event_logger.info(f"{update.message.from_user} tried to use the /get command from chat {update.message.chat_id}")
        return  
    ...
    if response.status_code == 200:
        ...
        context.bot.send_message(chat_id=update.message.chat_id, text=info, reply_to_message_id=update.message.message_id)  
    else:
        error_msg = f"Error {response.status_code}: Unable to retrieve information for stack {stack_name}"
        event_logger.error(error_msg)
        context.bot.send_message(chat_id=update.message.chat_id, text=error_msg, reply_to_message_id=update.message.message_id)


def set_stack_versions(update, context):
    # Check if the command is sent from the specific group
    if str(update.message.chat_id) != CHAT_ID:
        event_logger.info(f"{update.message.from_user} tried to use the /set command from chat {update.message.chat_id}")
        return
    if len(context.args) < 3 or not all([context.args[1].isdigit(), context.args[2].isdigit()]):
        ...
        if file_response.status_code != 200:
            event_logger.error(f"Failed to get stack file with status code {file_response.status_code}")
            context.bot.send_message(chat_id=update.message.chat_id, text=f"Failed to get stack file with status code {file_response.status_code}")
            return
        ...
        if env_response.status_code != 200:
            event_logger.error(f"Failed to get stack environment variables with status code {env_response.status_code}")
            context.bot.send_message(chat_id=update.message.chat_id, text=f"Failed to get stack environment variables with status code {env_response.status_code}")
            return
        ...
        if put_response.status_code == 200:
            event_logger.info(f"Set [{stack_name}]: front: {front_ver}, back: {back_ver}")
            reply_text = f"Set: {stack_name} {front_ver} {back_ver}"
            context.bot.send_message(chat_id=update.message.chat_id, text=reply_text, reply_to_message_id=update.message.message_id)
        else:
            event_logger.error(f"Failed to update stack versions with status code {put_response.status_code}")
            context.bot.send_message(chat_id=update.message.chat_id, text=f"Failed to update stack versions with status code {put_response.status_code}", reply_to_message_id=update.message.message_id)

def get_chat_id(update, context):
    chat_id = update.message.chat_id
    event_logger.info(f"Chat ID: {chat_id}")
    context.bot.send_message(chat_id=chat_id, text=f"This chat ID is: {chat_id}", reply_to_message_id=update.message.message_id)
...
def main():
    ...
    commands = [
        BotCommand('get', 'Returns <stack name> <frontend version> <backend version>'),
        BotCommand('set', 'Sets <stack name> <frontend version> <backend version>'),
